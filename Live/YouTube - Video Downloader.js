...
(function() {
    ...
    function autoDownload(url, filename, buttonElement, format) {
        console.log(`Auto-downloading: ${filename}`);
        
        // Open download in new tab (bypasses popup blocker)
        ...
        if (detailsDiv) {
            detailsDiv.innerHTML = `
                <span style="color: #2a6e3a;">✓ Download started!</span>
                <span class="yt-format-badge">Complete</span>
            `;
        }
            ...
            GM_xmlhttpRequest({
                ...
                onload: function(res) {
                    ...
                    try {
                        ...
                        // When download URL is ready, trigger auto-download immediately
                        if (data.download_url && data.download_url !== downloadUrl && !downloadTriggered) {
                            downloadTriggered = true;
                            const finalUrl = data.download_url;
                            const filename = `${sanitizeFilename(videoTitle)}_${format}.${getExtension(format)}`;

                            updateProgressDisplay(buttonElement, 100, ' Downloading...');
                            buttonElement.style.opacity = '1';
                            buttonElement.style.cursor = 'pointer';
                            
                            // AUTO DOWNLOAD - no second click needed!
                            autoDownload(finalUrl, filename, buttonElement, format);
                            return;
                        }
    ...
    function createFormatItem(format, type, videoUrl, videoTitle) {
        ...
        item.onclick = (e) => {
            e.stopPropagation();
            // Only start download if not already processing
    ...
    function injectButton() {
        ...
        if (lastVideoId === videoId && document.getElementById('yt-ultimate-download-btn')) {
            hidePremiumButton();
            return;
        }

        console.log(`Video changed to: ${videoId}, injecting button...`);
        lastVideoId = videoId;
        ...
        const findAndAdd = () => {
            ...
            for (const selector of selectors) {
                ...
                if (container && container.children.length > 0) {
                    // Prevent duplicates
                    if (container.querySelector('#yt-ultimate-download-btn')) {
                        return true;
                    }

                    const oldBtn = document.getElementById('yt-ultimate-download-btn');
    ...
    // Less aggressive interval fallback
    setInterval(() => {
        injectButton();
    }, 500);
